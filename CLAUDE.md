...
- **Develop on 3.13; the floor is 3.11.** Use `uv run --python 3.13 ...` for
  ...
  is why anything the suite needs belongs in a group, not behind an extra. The
  one exception is `ipopt`: cyipopt has no wheels and builds against a native
  IPOPT, so the Phase-2 tests run on trust-constr where it is absent. 3.11 is what `enum.StrEnum` needs;
