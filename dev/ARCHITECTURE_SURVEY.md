...
| D9 | The app and the optimizer resolve targets in opposite priority (fixed) | `planning.py:264-275` checks the inline RAS point first; `runtime/probe_context.py:36-58` checks `target_key` first | A plan setting both gets different targets in the app and the optimizer | verified |
