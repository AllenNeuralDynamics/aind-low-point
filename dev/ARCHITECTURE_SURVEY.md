...
| D8 | File-based calibrations always fail validation (fixed) | `config.py:1440-1442` forbids `reticle` with `file:`; `config.py:2003-2008` requires every calibration file's reticle to exist | Legacy `calibrations.files` entries with `file:` are unusable | verified |
