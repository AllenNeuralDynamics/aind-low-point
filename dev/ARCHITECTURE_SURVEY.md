...
| R1 | The app's FCL manager is shared across threads without a lock (fixed, `93256ed`) | The worker thread mutates the manager (`collisions.py:287-301`) while the kind-change path uses it on the main thread. `FCLBackend.sync` omits group and mask (`fcl_backend.py:63-79`). | A node added by `sync` was never reported as colliding at all, because it reached none of the group/mask maps | fixed |
| R2 | The AP/ML readout skips angle clamping (fixed, `458351f`) | `trame_controller.py:2106-2122` copies `planning.py:291-307` without `clamp_angles` | Sliders can show unclamped values | fixed |
| R3 | Default opacities bypass the material override path (fixed, `458351f`) | `trame_controller.py:2053-2063` sets actor opacity directly; any repaint restores the config value | Opacity resets on collision flips | fixed |
| R4 | The CCF overlay ignores lateralized and descendant labels (fixed, `d9fe08d`) | `ccf_overlay.py:66` meshes `volume == label_id` | Region overlays disagree with config-declared CCF assets | fixed |
| R5 | Two caches are keyed on object `id()` (fixed, `93256ed`) | `_SDF_JNP_CACHE` (`probe_static.py:74-97`), `_SDF_PACK_CACHE` (`phase1.py:812`) | Stale hits after garbage collection; unbounded growth. The correctness of one cache rested on the other never evicting. | fixed |
