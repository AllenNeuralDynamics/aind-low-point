...
| D12 | The thread-pool Phase 2 ignores `settings.warmup` (fixed) | `solve_candidates` warms unconditionally on the thread branch | `WARMUP=0` has no effect in the default pool mode | verified |
