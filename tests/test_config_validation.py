...
class TestConfigCrossReferenceValidation:
    ...
    def test_scene_asset_reference_validation(self):
        ...
        with pytest.raises(ValidationError, match="asset 'nonexistent_asset' not found in assets"):
    ...
    def test_scene_transform_reference_validation(self):
        ...
        with pytest.raises(ValidationError, match="transform key 'nonexistent_transform' not found"):
            ConfigModel.model_validate(config_data)
    ...
    def test_scene_pose_source_probe_validation(self):
        ...
        with pytest.raises(ValidationError, match="pose_source_probe 'nonexistent_probe' not in plan.probes"):
            ConfigModel.model_validate(config_data)
    ...
    def test_target_source_key_validation(self):
        ...
        config_data.update({
            "assets": [AssetFactory.mesh_asset(key="brain_mesh")],
            "targets": [
                TargetFactory.derived_target(key="valid_target", source_key="brain_mesh"),
                TargetFactory.derived_target(key="invalid_target", source_key="nonexistent_asset"),
            ],
        })

        with pytest.raises(ValidationError, match="source_key 'nonexistent_asset' not found in assets"):
            ConfigModel.model_validate(config_data)
    ...
    def test_probe_arc_reference_validation(self):
        ...
        with pytest.raises(ValidationError, match="arc 'nonexistent_arc' not found in plan.arcs"):
            ConfigModel.model_validate(config_data)
    ...
    def test_probe_target_reference_validation(self):
        ...
        with pytest.raises(ValidationError, match="target 'nonexistent_target' not found in targets"):
            ConfigModel.model_validate(config_data)
    ...
    def test_calibration_file_reticle_validation(self, temp_dir_path):
        ...
        config_data.update({
            "plan": {
                "arcs": {},
                "probes": {},
                "reticles": {"reticle1": {"offset_RAS": [0.0, 0.0, 0.0], "rotation_z": 0.0}},
                "calibrations": {
                    "files": {
        ...
        with pytest.raises(ValidationError, match="reticle 'nonexistent_reticle' not defined in plan.reticles"):
            ConfigModel.model_validate(config_data)

    def test_calibration_probe_to_ref_validation(self, temp_dir_path):
        """Test calibration probe_to_ref must reference existing probes and cal files."""
        ...
        config_data["plan"]["calibrations"] = {
            "files": {
                "cal1": CalibrationFactory.calibration_source_dir(str(cal_dir), reticle="reticle1")
            },
            "probe_to_ref": {
                "probe1": "cal1:12345",  # Valid: probe exists, cal file exists
                "nonexistent_probe": "cal1:67890",  # Invalid: probe doesn't exist
                "probe1_dup": "nonexistent_cal:12345",  # Invalid: cal file doesn't exist
            }
        }
    ...
    def test_canonicalization_ref_validation(self):
        """Test canonicalization_ref fields must reference existing canonicalizations."""
        config_data = ConfigFactory.minimal_config()
        config_data.update({
            "canonicalizations": {
            ...
            "assets": [
                AssetFactory.mesh_asset(key="valid_asset", canonicalization_ref="canon1"),
                AssetFactory.mesh_asset(key="invalid_asset", canonicalization_ref="nonexistent_canon"),
            ],
        })

        with pytest.raises(ValidationError, match="canonicalization_ref 'nonexistent_canon' not found"):
            ConfigModel.model_validate(config_data)
    ...
    def test_file_native_requires_transform(self):
        ...
        with pytest.raises(ValidationError, match="source_space=FILE_NATIVE requires a transform"):
            ConfigModel.model_validate(config_data)
    ...
    def test_valid_complete_config(self, sample_transform_file, temp_dir_path):
        ...
        config_data.update({
            ...
            "assets": [
                AssetFactory.mesh_asset(key="brain_mesh", canonicalization_ref="canon1")
            ],
            "targets": [
                TargetFactory.explicit_target(key="target1"),
                TargetFactory.derived_target(key="target2", source_key="brain_mesh"),
            ],
            "scene": {
                "nodes": [
                    {
