...
# ---------------------------------------------------------------------------
# bounded_isotonic_arc_aps — box-constrained isotonic arc-AP placement
# ---------------------------------------------------------------------------
...
def _max_violation(ap, lo, hi, c):
    """Max of (separation shortfall, box overflow) — 0 ⇒ feasible."""
    order = np.argsort(c)
...
def _qp_oracle(c, lo, hi):
    """Independent SLSQP solve of the same projection — feasible oracle."""
...
def test_bounded_isotonic_slack_unchanged():
    # Already ≥16° apart, wide boxes ⇒ stays at the centroids (non-packed).
...
def test_bounded_isotonic_binding_splits_deficit():
    # 5° apart ⇒ pushed to exactly 16°, centered on the mean (5).
...
def test_bounded_isotonic_box_clamp_stays_feasible():
    # arc2 capped at 18 ⇒ packing slides down to respect both box and sep.
    c = np.array([0.0, 5.0, 10.0])
...
def _feasible_instance(rng):
    ...
    true = true + rng.uniform(_LO - true.min(), _HI - true.max())  # rigid shift
