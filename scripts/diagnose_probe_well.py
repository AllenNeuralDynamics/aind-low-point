...
def main() -> int:
    ...
    print(f"well SDF grid: {well.grid.shape}, "
    ...
    R_p, t_p = pose_from_optimizer_vars(
        target_LPS=sb.target_LPS, ap_deg=ap, ml_deg=ml,
        spin_deg=spin_deg,
    ...
    hard, soft = pairwise_signed_clearance_probe_fixture_body(
        ...
        well.grid, well.origin, well.spacing,
        jnp.asarray(probe_sdf.surface_points, dtype=jnp.float32),
        well.surface,
    )
    print(f"JAX clearance probe_fixture_body: hard={float(hard):+.4f}, soft={float(soft):+.4f}")

    # === Look at FCL contact points and check both SDFs ===
    print(f"\n{'=' * 70}")
    print(f"FCL contact points → both SDFs")
    print(f"{'=' * 70}")
    cr = fcl.CollisionResult()
    fcl.collide(sb.bvh_obj, well_bvh,
                fcl.CollisionRequest(num_max_contacts=8, enable_contact=True), cr)
    ...
    for k, c in enumerate(cr.contacts or []):
        ...
        d_in_probe = float(np.asarray(trilinear_sdf(
            jnp.asarray(probe_sdf.grid, dtype=jnp.float32),
            jnp.asarray(probe_sdf.origin, dtype=jnp.float32),
            jnp.asarray(probe_sdf.spacing, dtype=jnp.float32),
    ...
    if len(di) > 0:
        ...
        print(f"  {n_inside}/{len(di)} probe samples have well-SDF < 0 (inside well envelope)")
        print(f"  min well-SDF over probe samples: {di.min():+.4f} mm")
        print(f"  10 deepest: {sorted(di)[:10]}")
    else:
        print(f"  no probe surface samples fell inside well's SDF grid bbox")
    ...
    d_w_in_p = np.asarray(trilinear_sdf(
        jnp.asarray(probe_sdf.grid, dtype=jnp.float32),
        jnp.asarray(probe_sdf.origin, dtype=jnp.float32),
        jnp.asarray(probe_sdf.spacing, dtype=jnp.float32),
    ...
    if len(di) > 0:
        ...
        print(f"  no well surface samples fell inside probe's SDF grid bbox")
