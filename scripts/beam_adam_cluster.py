...
def run_cand(idx, *, rt, probes, holes, sdf_by_name, bvh, fixtures, well,
             fixture_bvhs, pool, brain_sdf, probe_kind, mesh_by_kind, names, K):
    ...
    n = len(spin_seeds)
    basins = {_bin([float(np.degrees(np.arctan2(
        xa[b][n_arcs + 6 * k + 2], xa[b][n_arcs + 6 * k + 1]))) for k in range(K)]): b
        for b in range(n)}
    feas = []
    for b in range(n):
    ...
    return dict(idx=idx, n_basins=len(basins), n_feas=len(feas),
                beam_best_cov=best_cov, beam_best_fcl=best_fcl,
                restore_cov=cov(xr), restore_fcl=rfcl, restore_feas=rfcl >= -1e-4,
                durable_cov=cov(xd), durable_fcl=dfcl, durable_feas=dfcl >= -1e-4)
...
def main() -> int:
    ...
    print(f"{'idx':>5}  | {'cov':>6} {'fcl':>7} {'':>4} | "
          f"{'basins':>6} {'feas':>4} {'best_cov':>8} {'best_fcl':>8} | verdict")
    for idx in IDXS:
        r = run_cand(idx, rt=rt, probes=probes, holes=holes,
                     sdf_by_name=sdf_by_name, bvh=bvh, fixtures=fixtures, well=well,
                     fixture_bvhs=fixture_bvhs, pool=pool, brain_sdf=brain_sdf,
                     probe_kind=probe_kind, mesh_by_kind=mesh_by_kind, names=names,
                     K=K)
        dwin = r["restore_cov"] - r["beam_best_cov"]
        verdict = (f"restore +{dwin:.2f} cov" if dwin > 0.05
                   else f"beam +{-dwin:.2f} cov" if dwin < -0.05 else "tie")
        rfeas = "FEAS" if r["restore_feas"] else "infes"
        print(f"{idx:>5}  | {r['restore_cov']:>6.2f} {r['restore_fcl']:>+7.3f} "
