...
RANKS = [
    int(x) for x in _os.environ.get("RANKS", "0,1,4,7,12,30,100,500,2000").split(",")
]
...
def main() -> int:
    ...
    print(
        f"Phase 2 trust-constr maxiter={P2_ITER} min_clear={P2_MINCLEAR} "
        f"lam_clear={P2_LAM_CLEAR} tau_clear={P2_TAU_CLEAR}"
    )
    print(
        f"{'rank':>5} {'cand':>6} {'min':>6} {'it':>4} {'s/it':>6} "
    ...
    for ri in RANKS:
        ...
        print(
            f"{ri:>5} {r['idx']:>6} {t_min:>6.0f}s {res.nit:>4} "
