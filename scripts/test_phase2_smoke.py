...
def main() -> int:
    ...
    print(
        f"[Phase 2] x1 (Phase 1 endpoint): obj={obj0:.4f}, "
    ...
    print(
        f"[Phase 2] fn={obj_end:.4f}, iter={res2.nit}, "
        f"success={res2.success}, status={res2.status}"
    )
    print(f"          message: {res2.message}")
    print(
        f"[Phase 2] x2 slacks: min={slacks_end.min():+.4f}, "
    ...
    if slacks_end.min() >= -1e-4:
        ...
        print(
            f"[Phase 2 verdict] left {int(np.sum(slacks_end < 0))} "
    ...
    print(
        f"          FCL slacks at x2: min={fcl_s.min():+.4f}, "
    ...
    if fcl_s.min() >= -1e-4:
        ...
        print(
            f"[FCL verdict] {int(np.sum(fcl_s < 0))} FCL violations "
            f"(min {fcl_s.min():+.4f})."
        )
