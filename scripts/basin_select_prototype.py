...
def main() -> int:
    ...
    print(
        f"sample: top-{N_SAMPLE} by Stage-2 max_viol + manual "
        f"(#{MANUAL_IDX}); {len(sample)} cands, {N_BASINS} basins each\n"
    )

    viol_w = Phase1Weights(
    ...
    for n_done, idx in enumerate(sample):
        ...
        p1_viol, _ = make_phase1_objective(
            statics, n_arcs, coverage_data=None, fixtures=fixtures, weights=viol_w
        )
        ...
        print(
            f"[{n_done + 1}/{len(sample)}] cand {idx:5d}  "
    ...
    for r in rows[:10]:
        ...
        print(
            f"{r[0]:>6} {r[1]:>11.1f} {r[2]:>9.2f} {r[3]:>+8.3f} "
