...
def main() -> int:
    ...
    print(
        f"  enumeration: {time.perf_counter() - t0:.1f}s "
    ...
    if args.n_cands is not None and args.n_cands < len(candidates):
        ...
        if manual_rank is not None and manual_rank >= args.n_cands:
            ...
            print(
                f"  swapped slot {args.n_cands - 1} for manual (rank "
        ...
        manual_rank = next(
            (
                ...
                if MANUAL_H_836656_T12
                == {p: c.ha.probe_to_hole[p] for p in MANUAL_H_836656_T12}
            ),
            None,
        )
        print(
            f"  limited to {len(candidates)} candidates for probe run "
            f"(manual is rank {manual_rank} in probe)",
            flush=True,
        )
    ...
    print(f"  SDFs + pose features: {time.perf_counter() - t0:.1f}s", flush=True)
    ...
    if args.spin_restore_chunk == "auto":
        ...
        chunk = estimate_spin_restore_chunk(
            ...
            K=len(probes),
            total_B=len(candidates),
            safety_margin=args.vram_safety_margin,
        )
        print(f"  spin_restore_chunk=auto → {chunk} (from free VRAM probe)")
    else:
    ...
    print(
        f"  max_viol  min={max_viols.min():.4f} "
        f"median={float(np.median(max_viols)):.4f} "
        f"max={max_viols.max():.4f}"
    )

    # Sort by lex_key, report top-10
    sorted_results = sorted(enumerate(results), key=lambda kv: kv[1].metrics.lex_key())
    print()
    print("Top-10 by lex_key (max_viol, sum_viol², -coverage):")
    print(
        f"  {'rank':>4} {'cand#':>6} {'max_viol':>10} {'sum_viol²':>12} "
        f"{'coverage':>10}"
    )
    for rank, (cand_idx, jc) in enumerate(sorted_results[:10]):
        ...
        if manual_rank is not None and cand_idx == manual_rank:
            marker = "  [MANUAL]"
        print(
            f"  {rank:>4d} {cand_idx:>6d} {m.max_violation:>10.4f} "
    ...
    if manual_rank is not None:
        ...
        manual_rank_post = next(
            (
                rank
                for rank, (idx, _) in enumerate(sorted_results)
                if idx == manual_rank
            ),
            None,
        )
    ...
    if not args.skip_augment:
        ...
        subprocess.run(
            [
                ...
                "-u",
                "-m",
                "scripts.augment_polish_with_offsets",
                str(args.config),
                str(args.holes),
                "--in-pkl",
                str(args.output),
        ...
        subprocess.run(
            [
                ...
                "-u",
                "-m",
                "scripts.eval_violation_at_augmented",
                str(args.config),
                str(args.holes),
                "--in-pkl",
                str(args.output),
