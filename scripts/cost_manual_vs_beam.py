...
def plan_to_x(plan_data: dict, statics: list, n_arcs: int,
              probe_to_arc_idx: dict[str, int]) -> np.ndarray:
...
def cost_with_field_zeroed(x: np.ndarray, statics: list, n_arcs: int,
                            coverage_data, fixtures,
                            field: str | None) -> float:
    ...
    """
    w = Phase1Weights() if field is None else replace(
        Phase1Weights(), **{field: 0.0}
    )
    fn, _ = make_phase1_objective(
        statics, n_arcs, coverage_data=coverage_data,
        fixtures=fixtures, weights=w,
    )
    return float(fn(x))
...
def main() -> int:
    ...
    plans = {
        "manual": ("examples/836656-config-T12.plan.yml", manual_plan)
    }
    ...
    for label, (path, plan_data) in plans.items():
        ...
        total = cost_with_field_zeroed(
            x, statics, n_arcs, coverage_data, fixtures, field=None,
        )
        deltas: dict[str, float] = {}
        for short, attr in fields:
            v = cost_with_field_zeroed(
                x, statics, n_arcs, coverage_data, fixtures, field=attr,
            )
