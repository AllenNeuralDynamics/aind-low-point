...
from aind_low_point.optimization.stage3_phase1_jax import (
    PHASE1_PER_PROBE_VARS, Phase1Weights,
)
...
def adam_snap(x0, grad_fn, obj_fn, lo, hi, lr, snaps, b1=0.9, b2=0.999, eps=1e-8):
    ...
    lo = jnp.asarray(lo, jnp.float32); hi = jnp.asarray(hi, jnp.float32)
    out = {}
...
def main() -> int:
    ...
    print(f"{'lr':>6} {'steps':>6} {'man_rank':>9} {'feas_in_top3':>13} "
    ...
    for lr in LRS:
        ...
        for s in SNAPS:
            ...
            print(f"{lr:>6.3f} {s:>6} {man_rank:>9} "
