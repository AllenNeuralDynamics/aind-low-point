...
def main() -> int:  # noqa: C901
    ...
    statics = _build_probe_static(
        probes, holes, cand.ha, cand.aa, bvh_cache=None, sdf_by_name=None
    )
    ...
    for kind, (zc, rmax, z_lo, z_hi) in prof_by_kind.items():
        ...
        print(
            f"  {kind:<18} r_max={rmax.max():.2f} mm  r_median={rmed:.2f} mm  "
    ...
    for (i, j), g in sorted(gaps.items(), key=lambda kv: kv[1]):
        print(
            f"    {names[i]:>4}↔{names[j]:<4} {g:+6.2f}{'  COUPLED' if g < 0 else ''}"
        )
    ...
    print(
        "\nunified facing (narrow profile → tightest coupled partner; probes + well):"
    )
    for i, st in enumerate(statics):
        ...
        wstr = " ".join(
            f"{lab}={(t / t_max) ** ALPHA_FACING:.2f}" for lab, t, _ in partners
        )
        print(
            f"  {names[i]:<5} tightest={lab0:<4}(pen={t_max:.2f})  "
    ...
    for i, st in enumerate(statics):
        h1 = spin_to_align_y_with(
            st.assigned_hole.slot_major_dir(), float(arc_aps[st.arc_idx]), float(ml[i])
        )
        print(
            f"  {names[i]:<5}  slot-aligned(H1)={_wrap(h1):+7.1f}  "
            f"manual={manual[i]:+7.1f}  cands={[round(c) for c in spin_cands[i]]}"
        )

    beam = beam_search_assignments(
        statics, spin_cands, coupling, target_LPS, arc_aps, ml, probe_kind, beam_B=64
    )
    print(
    ...
    for r, asg in enumerate(beam[:10], 1):
        ...
        print(
            f"  #{r:<2} score={asg.score:6.2f}  spins={sv}"
