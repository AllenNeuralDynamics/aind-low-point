...
MANUAL = {"MD": -34.0, "BLA": 0.0, "PL": 131.0, "VM": -180.0,
          "RSP": 4.0, "CA1": 87.0, "CLA": 171.0}
PHASE1_PER_PROBE_VARS = 6


def adam(x0, grad_fn, lo, hi, *, steps=600, lr=0.02, b1=0.9, b2=0.999,
         eps=1e-8):
...
def main() -> int:
    ...
    validator = make_fcl_validator(statics, n_arcs, fixtures=(well,),
    ...
    spin_cands = per_probe_spin_candidates(
        statics, coupling, target_LPS, arc_aps, mls, {p.name: p.kind for p in probes},
        seed_spins={i: float(spin_aug[i]) for i in range(n_probes)})
    beam = beam_search_assignments(statics, spin_cands, coupling, target_LPS,
    ...
    x0 = np.stack([build_y(arc_aps, n_arcs, mls, sp, zero, zero, zero)
                   for _, sp in basins])  # (B, nv)
    B = x0.shape[0]
    ...
    fun, jac = make_phase1_objective(statics, n_arcs, coverage_data=None,
                                     fixtures=(well,), weights=weights)
