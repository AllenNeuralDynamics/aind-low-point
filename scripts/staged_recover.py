...
def seed_ml(cand, names, enum):
    ...
    m = mrv_greedy_ml(enum, cand.ha.probe_to_hole, cand.aa.probe_to_arc_idx)
    ...
    def run_one(args):
        ...
        try:
            res = minimize(fun, np.asarray(x0, np.float64), method="L-BFGS-B",
                           jac=jac, bounds=bnds, options=opts)
...
def run_stage(statics_flat, x0_rows, n_arcs, K, *, coverage_data, fixtures,
              bounds_list, iters, well_obj):
    """Dispatch a stage to L-BFGS (per-row bounds) or batched ADAM (shared
    bounds only — used for the no-offset final check)."""
    if OPT == "lbfgs":
        return lbfgs_pass(statics_flat, x0_rows, n_arcs,
                          coverage_data=coverage_data, fixtures=fixtures,
                          bounds_list=bounds_list, max_iter=iters)
    ...
    out = adam_pass(statics_flat, x, n_arcs, coverage_data=coverage_data,
                    well_obj=well_obj, bounds=b0, steps=iters)
...
def main() -> int:
    ...
    tag = (f"{OPT}_ml-{ML_SEED}_s1well-{int(S1_WELL)}_rwell-{int(RESTORE_WELL)}"
           f"_off-{OFFSET_ITERS}_n{len(idxs)}")
    print(f"=== staged recover [{tag}] ===")
    print(f"opt={OPT} stage1={STAGE1} stage2={STAGE2} offset_iters={OFFSET_ITERS} "
    ...
    for n_arcs in sorted(by_arcs):
        ...
        x1 = run_stage(statics_flat, x0_rows, n_arcs, K, coverage_data=None,
                       fixtures=s1_fix, bounds_list=[red_b] * len(g),
                       iters=s1_iters, well_obj=well if S1_WELL else None)
        ...
        if OFFSET_ITERS > 0:
            ...
            x1 = lbfgs_pass(statics_flat, x1, n_arcs, coverage_data=cov_data,
                            fixtures=(well,), bounds_list=off_b,
                            max_iter=OFFSET_ITERS)

        # Stage 2: full, coverage on, all DOF free.
        full_b = phase1_bounds(n_arcs, K)
        x2 = run_stage(statics_flat, x1, n_arcs, K, coverage_data=cov_data,
                       fixtures=(well,), bounds_list=[full_b] * len(g),
                       iters=s2_iters, well_obj=well)
        ...
        for ci, idx in enumerate(g):
            ...
            ofun, _ = make_phase1_objective(
                st, n_arcs, coverage_data=cov_data, fixtures=(well,),
    ...
    print(f"    overlap (both feasible):    {len(overlap)}/{len(dur_feas)} "
    ...
    with open(f"scratch/staged_recover_{tag}.pkl", "wb") as f:
        pickle.dump({"rows": rows, "tag": tag, "ml_seed": ML_SEED, "opt": OPT,
                     "stage1": STAGE1, "stage2": STAGE2,
                     "offset_iters": OFFSET_ITERS, "s1_well": S1_WELL,
                     "restore_well": RESTORE_WELL}, f)
    return 0
