    ...
    for h in holes:
        ...
        new_sections = [
            HoleSection(
                ...
                a=s.a, b=s.b, theta=s.theta,
            )
            for s in h.sections
        ]
        out.append(Hole(id=h.id, axis=new_axis, ref_point=new_ref,
                        sections=new_sections))
    return out


def _probe_static_info(
    plan_state, runtime, name: str
) -> ProbeStaticInfo:
...
def _apply_result_to_plan_state(
    plan_state, result: OptimizationResult
) -> None:
...
def main():
    ...
    probes = [
        _probe_static_info(plan_state, runtime, name)
    ...
    print(f"  breakdown: coverage={result.breakdown.coverage_total:.3f}, "
    ...
    with open(out_path, "w") as f:
        yaml.safe_dump(new_cfg.model_dump(mode="json"), f,
