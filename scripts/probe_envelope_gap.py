...
def main(cand_idx=1042, probe_a="BLA", probe_b="RSP", offset_mm=0.2):
    ...
    sa_st.bvh_obj.setTransform(
        fcl.Transform(
            np.ascontiguousarray(Ra),
            np.ascontiguousarray(ta),
        )
    )
    sb_st.bvh_obj.setTransform(
        fcl.Transform(
            np.ascontiguousarray(Rb),
            np.ascontiguousarray(tb),
        )
    )
    ...
    fcl.collide(
        sa_st.bvh_obj,
        sb_st.bvh_obj,
        fcl.CollisionRequest(num_max_contacts=20, enable_contact=True),
        cr,
    )
    ...
    env_a_vals = np.asarray(
        trilinear_sdf(
            grid_a,
            origin_a,
            spacing_a,
            jnp.asarray(local_a, dtype=jnp.float32),
        )
    )
    env_b_vals = np.asarray(
        trilinear_sdf(
            grid_b,
            origin_b,
            spacing_b,
            jnp.asarray(local_b, dtype=jnp.float32),
        )
    )

    for i, (p, ea, eb, ma, mb) in enumerate(
        zip(
            contact_pts,
            env_a_vals,
            env_b_vals,
    ...
    print("\nLegend:")
    print(
        "  env_*    = JAX trilinear SDF of α-wrap envelope at the contact "
        "(in that probe's local frame)"
    )
    print(
        "             EXPECTED ~−offset_mm (=−0.2) if envelope is "
        "conservative outside true mesh"
    )
    print(
        "  mesh_*   = trimesh.proximity.signed_distance from raw mesh "
        "(negative = inside)"
    )
    print("             Contacts are ON raw surface, so |mesh| should be ≈ 0")
    print("  env−mesh = how much envelope SDF differs from true mesh distance")
    print("             EXPECTED ≈ −offset_mm (−0.2) — envelope is INFLATED outward")
    print(
        "             If POSITIVE: envelope is SHRUNKEN (inside true mesh) — the bug."
    )
