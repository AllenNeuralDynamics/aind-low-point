...
def mrv_greedy_ml(enum, probe_to_hole, probe_to_arc_idx):
    """New-MRV greedy-stab ml seed for a fixed (hole, arc) decision.

    Mirrors the enumerator's emit: per arc, intersect member (probe,hole) AP
    envelopes, take each member's ml-window over that AP intersection, then
    greedy-pack at MIN_ML_SEP_DEG. Falls back to window midpoints if the arc
    isn't greedy-packable (the atlas-range pack can be tighter than SLSQP).
    """
    ...
    for members in arcs.values():
        lo = max(enum.arr.ap_min_max[(p, h)][0] for p, h, _ in members)
        hi = min(enum.arr.ap_min_max[(p, h)][1] for p, h, _ in members)
        windows = [enum.ml_window(p, h, lo, hi) for p, h, _ in members]
        pts = greedy_place(windows, MIN_ML_SEP_DEG)
        if pts is None:
            pts = [0.5 * (w[0] + w[1]) for w in windows]
        for (p, _h, name), ml in zip(members, pts):
...
def adam_pass(statics_flat, x0, n_arcs, *, coverage_data, well_obj, bounds,
    ...
    _vobj, _vg, build_arglist, make_adam = make_batched_phase1_chunked(
        statics_flat[0], n_arcs, Phase1Weights(), fixtures,
    ...
    x0p = (np.concatenate([x0, np.repeat(x0[-1:], n_pad, 0)], 0)
           if n_pad else x0).astype(np.float32)
...
def main() -> int:
    ...
    print(f"staged ADAM (reduced {STAGE1} → full {STAGE2}); NO L-BFGS; "
    ...
    for n_arcs in sorted(by_arcs):
        g = by_arcs[n_arcs]
        print(f"[n_arcs={n_arcs}] {len(g)} cands: restore → reduced ADAM "
              f"→ full ADAM ...", flush=True)
        ...
        x1 = adam_pass(statics_flat, x0, n_arcs, coverage_data=None,
                       well_obj=well if S1_WELL else None,
                       bounds=reduced_bounds(n_arcs, K), steps=STAGE1)
        x2 = adam_pass(statics_flat, x1, n_arcs, coverage_data=cov_data,
                       well_obj=well, bounds=phase1_bounds(n_arcs, K),
                       steps=STAGE2)
