...
from scripts.spin_heuristic_search import (
    beam_search_assignments, build_coupling_graph, is_four_shank,
    per_probe_spin_candidates, spin_to_align_y_with,
)
...
def main() -> int:
    ...
    for cand_idx in (4195, 1035):
        ...
        h1 = np.array([
            spin_to_align_y_with(st.assigned_hole.slot_major_dir(),
                                 float(arc_aps[st.arc_idx]), float(mls[i]))
        ...
        sc = per_probe_spin_candidates(
            statics, coupling, np.array([st.target_LPS for st in statics]),
            arc_aps, mls, {p.name: p.kind for p in probes},
            seed_spins={i: float(spin_inc[i]) for i in range(n_probes)})
        beam = beam_search_assignments(
            statics, sc, coupling, np.array([st.target_LPS for st in statics]),
            arc_aps, mls, {p.name: p.kind for p in probes}, beam_B=16)
        basin_sets["beam4"] = [np.array([dict(a.spins)[i]
                               for i in range(n_probes)]) for a in beam[:4]]
        ...
        fcls = np.array([float(np.asarray(validator.slacks(x_adam[i])).min())
                         for i in range(B)])
        ...
        for name in basin_sets:
            ...
            print(f"  {name:<10} best FCL {best_fcl:>+7.3f}  "
                  f"({feas}/{mask.sum()} basins feasible)")
        print(f"  -> cheap4 best {best['cheap4']:+.3f} vs beam4 best "
              f"{best['beam4']:+.3f}  "
