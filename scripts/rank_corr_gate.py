...
def main() -> int:
    ...
    idxs = list(np.asarray(eligible)[
        np.linspace(0, len(eligible) - 1, N_SAMPLE).astype(int)])
    ...
    t_adam = time.time() - t0
    print(f"  {t_adam:.1f}s total ({t_adam/len(idxs)*1000:.0f} ms/cand incl compile)")
    ...
    for i, st in enumerate(statics_list):
        fun, jac = make_phase1_objective(st, n_arcs, coverage_data=None,
                                         fixtures=(well,), weights=weights)
        r = minimize(fun, y0[i], jac=jac, method="L-BFGS-B", bounds=bounds,
    ...
    validator = make_fcl_validator(statics_list[0], n_arcs, fixtures=(well,),
    ...
    for j in adam_rank[:8]:
        idx = idxs[j]
        v = make_fcl_validator(statics_list[j], n_arcs, fixtures=(well,),
                               fixture_bvhs=fixture_bvhs)
        fcl = float(np.asarray(v.slacks(x_adam[j])).min())
        tag = " <-- MANUAL" if idx == 4195 else ""
        print(f"{idx:>6} {adam_viol[j]:>10.3f} {scipy_viol[j]:>11.3f} "
              f"{fcl:>+9.3f}{tag}")
    ...
    for j in dis:
        print(f"  cand {idxs[j]:>5}: adam_rank {ar[j]:>3} vs scipy_rank {sr[j]:>3}"
