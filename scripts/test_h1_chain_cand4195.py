...
def h1_closest_to(cands: list[float], target_deg: float,
                  four_shank: bool) -> float:
    ...
    if four_shank:
        ...
        errs = [min(abs(_wrap_deg(target_deg - c)),
                    abs(_wrap_deg(target_deg - c - 180.0))) for c in cands]
    else:
...
def build_y(arc_aps: np.ndarray, n_arcs: int, mls: np.ndarray,
            spins_deg: np.ndarray, offsets_R: np.ndarray,
            offsets_A: np.ndarray, depths: np.ndarray) -> np.ndarray:
    n_probes = len(mls)
...
def run_chain(y0: np.ndarray, statics: list, n_arcs: int,
              coverage_data, fixtures, validator,
...
def main() -> int:
    ...
    for i, st in enumerate(statics):
        ...
        print(f"  {st.name:<5} ({kind:<18}, 4S={str(four_shank):<5})  "
    ...
    for i, st in enumerate(statics):
        print(f"  {st.name:<6} {seeds['pkl_seed'][i]:+8.2f} "
    ...
    for label, (_, s_fcl, feas, cov, _, wall) in results.items():
        n_viol = int((s_fcl < -1e-4).sum()) if s_fcl.size else 0
        tag = "FEAS" if feas else "FAIL"
        print(f"  {label:<10}: {tag}  fcl_min={s_fcl.min():+.4f}  "
