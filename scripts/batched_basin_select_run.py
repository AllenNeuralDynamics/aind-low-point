...
def main() -> int:
    ...
    fixture_bvhs = {well.name: make_fcl_bvh(
        runtime.asset_catalog.get_geometry(well.name).raw)}
    ...
    idxs = list(np.asarray(eligible)[
        np.linspace(0, len(eligible) - 1, N_CAND).astype(int)])
    if 4195 not in idxs:
        idxs[-1] = 4195
    ...
    print(f"Chunked COMPILED ADAM (chunk={CHUNK}, {STEPS} steps, lr 0.02, "
          f"{n_basin} basins)...")
    ...
    vobj, vgrad, build_arglist, make_adam = make_batched_phase1_chunked(
        statics_flat[0], n_arcs, Phase1Weights(), (well_obj,), coverage_data=None,
    ...
    print(f"  {t_adam:.1f}s total; first-chunk(compile) {t_first:.1f}s; "
          f"steady {steady:.2f}s/chunk = {steady/CHUNK*n_basin*1000:.1f} ms/cand")
    print(f"  VRAM: peak {peak/1e9:.2f} GB / limit {limit/1e9:.2f} GB; "
          f"~{per_row/1e6:.1f} MB/row; chunk @80% ≈ {sugg} rows")
    print(f"  full-pool est (8908 x {n_basin} basins, this chunk): "
    ...
    if PROFILE_CHUNK:
        ...
        print(f"PROFILE chunk={CHUNK} peak={peak} baseline={baseline} "
    ...
    print(f"Manual (#4195): basin-sel viol {best_viol[man_pos]:+.3f}, "
          f"rank {order[man_pos]+1}/{len(idxs)} "
          f"(frozen violation_fn was 676 / pool-rank #4641)")
    print(f"candidates with best_viol < 0 (soft-feasible-ish): "
    ...
    for k in range(min(FCL_TOPK, len(idxs))):
        ...
        v = make_fcl_validator(st, n_arcs, fixtures=(well,),
        ...
        if k < 12 or idxs[c] == 4195:
            ...
            print(f"  rank {k+1:>2}  cand {idxs[c]:>5}  viol {best_viol[c]:>+8.3f}  "
